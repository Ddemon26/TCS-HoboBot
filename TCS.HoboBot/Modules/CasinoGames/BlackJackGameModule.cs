...
{
    /* ───────────  card / shoe / hand models  ─────────── */

    enum Suit { Clubs, Diamonds, Hearts, Spades }

    // every rank now has a *unique* underlying integer (no duplicates)
    enum Rank
    {
    ...
    public async Task BlackjackAsync(float bet)
    {
        ...
        //    is funnelled through UpdateGameMessage so settlement & cleanup always run
